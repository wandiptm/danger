...
function urlonly(){var urlonly=new Array();setting.urlonly=setting.urlonly;urlonly=setting.urlonly.split(",");return urlonly;}
...
function showurl(datajson){
	...
	var urlonlylength = urlonly.length;
	var checklink = "";
	var checkexception = "";
	var linktag = document.getElementsByTagName("a");

	if (setting.exceptionurlstatus == false && setting.urlonlystatus == false) {
	...
	for (var i = 0; i < linktag.length; i++) {
		...
		checkexceptionurl = true;
		checkurlonly = false;
		no = 0;
		if (setting.exceptionurlstatus) {
			checkexceptionurl = false;
			while (checkexceptionurl == false && no < exceptionlength) {
		...
		if (setting.urlonlystatus) {
			checkurlonly = false;
			while (checkurlonly == false && no < urlonlylength) {
				checklink = extractDomain(linktag[i].href);
				checkexception = extractDomain(urlonly[no]);
				if (checklink.match(checkexception)) {
					checkurlonly = true;
				}
				no++;
		...
		if (checkexceptionurl == false || checkurlonly == true) {
